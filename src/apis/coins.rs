...
pub async fn coins(
    ...
    let provider_arc = match provider.lock() {
        Ok(guard) => guard.network.clone(),
        Err(poisoned) => poisoned.into_inner().network.clone(),
    };

    if let Some(network) = provider_arc {
        ...
        let blk_number = network.provider.get_block_number().await?.as_u64();
        ...
        let dive_contract_address = network.config.dive_contract_address;
        ...
        let mut ctx = provider.lock().unwrap();
